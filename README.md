...
3. Run `npm install`.
4. Run `npm start`.
5. Open `http://localhost:3000` or your network IP address (use your `PORT` value if changed in `.env`).

The server binds to 0.0.0.0, so it's accessible from other devices on your network at `http://<your-ip>:<PORT>`. (default is 3000).
